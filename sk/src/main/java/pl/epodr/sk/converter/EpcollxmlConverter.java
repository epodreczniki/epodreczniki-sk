...
@Service
public class EpcollxmlConverter extends AbstractConverter {

	private static final String SCRIPT = "collection2epcollxml.py -i %s -o %s -w %s";
	...
	protected void convert(File collectionWorkingDir, String variant, File womiDir, Killable parent) {
		...
		try {
			File tmpDir = Files.createTempDirectory("coll2coll-").toFile();
			pythonRunner.runScript(String.format(SCRIPT, workingDir, tmpDir, womiDir), parent);
